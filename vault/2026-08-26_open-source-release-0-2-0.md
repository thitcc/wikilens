...
## Approach
Five PRs off `main` (the fifth added 2026-09-04), in this order; every one
merges by merge commit after `/check` and CI.
...
5. **`chore/third-party-licenses`** (added 2026-09-04, merges between 3 and
   4) — the license texts of every shipped component: `THIRD-PARTY-LICENSES.txt`
   generated by `npm run licenses` (cargo-about over `src-tauri/about.toml`
   for the 277 Windows-target crates + a zero-dependency walk of
   `package-lock.json` for the 89 production npm packages), committed,
   shipped next to the exe through `bundle.resources`, byte-checked by a
   fourth CI job, pinned by a guardrail test, and pointed at from README,
   CONTRIBUTING, the notices file, the release-build skill and smoke item 11.
...
- License texts ship as a bundled resource next to the exe, not as
  `bundle.licenseFile` (an EULA page in both installers): `bundle.license`
  is only an identifier, Vite strips React's `@license` header, and the
  277 statically linked crates carry the same notice obligation. The
  inventory is committed and CI byte-checks it (a dependency change can't
  ship without its notice); the app crate is excluded (`publish = false` +
  `private.ignore`) so the release PR stays bump-only. The npm half is a
  lockfile walk rather than `generate-license-file`, which would pull
  arborist/pacote/sigstore into every `npm ci` and the audit gate.
...
- 2026-09-04 — branch 5 opened after an external review of the four PRs
  flagged that no third-party license text shipped (React's header is
  stripped from the bundle; the crates were never listed). Verified on the
  local build: the MSI's administrative extract places
  `THIRD-PARTY-LICENSES.txt` beside `wikilens.exe`; 277 crates, 89 packages,
  288 KB, byte-identical across regenerations. An adversarial review of the
  branch caught cargo-about harvesting `src/*.rs` files as the license text
  of encoding_rs and schemars_derive — now pinned by sha256 in `about.toml`,
  with the script refusing any text that contains Rust items, and crates
  counted once even when listed under two texts.
