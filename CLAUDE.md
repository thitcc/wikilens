...
- **Front-end changes with open visual choices get a proof sheet, decided
  mid-session** — full recipe in the `proof-sheet` skill
  (`.claude/skills/proof-sheet/SKILL.md`). Never end the turn to wait for a
  fresh prompt while a proof-sheet decision is pending: sheet → picks →
  implement is one continuous loop.
